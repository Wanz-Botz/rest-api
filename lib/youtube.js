		...
		if (ytIdRegex.test(link)) {
			...
			axios('https://www.y2mate.com/mates/en68/analyze/ajax', {
				...
				.then(({ data }) => {
					...
					const size = $('#mp4 > table > tbody > tr:nth-child(3) > td:nth-child(2)').text()
					const size_mp3 = $('#audio > table > tbody > tr:nth-child(1) > td:nth-child(2)').text()
					const id = /var k__id = "(.*?)"/.exec(data.result)[1]
					const configs = {
						type: 'youtube',
						_id: id,
						v_id: url[1],
